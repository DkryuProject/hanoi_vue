<template>
<v-card>
  <v-card-title>Menu Info</v-card-title>
  <v-card-title>
    ...
    <v-spacer></v-spacer>
    <div>
      <v-btn @click="$router.push('/menuInfoRegister')">NEW</v-btn>
      <v-btn @click="modifyData()">MOD</v-btn>
      <v-btn v-show="this.$store.state.role == 'ADMIN'" @click="deleteData()">DEL</v-btn>
    </div>
  </v-card-title>
  <v-divider></v-divider>
  <v-data-table v-model="selectedData" height="600px" fixed-header :headers="headers" :items="menuDetail" :items-per-page="10" :search="search" :single-select="false" :options.sync="options" :server-items-length="totalItem" item-key="menuSelectKey" show-select>
  </v-data-table>
</v-card>
</template>
  ...
  methods: {
    getItem() {
      ...
      axios.get('/api/v1/menu/getItem', {
        ...
        .then((response) => {
          this.menuDetail = response.data.content;
